(function() {
  var a = {},
    ...
    c = function(f, g) {
      ...
      if (!(j = a[h] || a[h = d(h, "./index")])) throw "module '" + f + "' not found";
      i = {
        id: h,
        exports: {}
      };
      try {
        return b[h] = i, j(i.exports, function(a) {
          return c(a, e(h))
        }, i), i.exports
      } catch (k) {
        throw delete b[h], k
    ...
    d = function(a, b) {
      ...
      for (var f = 0, g = d.length; f < g; f++) e = d[f], e == ".." ? c.pop() : e != "." && e != "" && c.push(e);
      return c.join("/")
    },
    e = function(a) {
  ...
  "base-platform/dom": function(a, b, c) {
    ...
    c.exports = {
      ...
      textContent: function(a, b) {
        if (b === undefined) return a.nodeType === 1 ? a.textContent !== undefined ? a.textContent : this.getChildren(a).map(function(a) {
          return this.textContent(a)
        }, this).join("") : a.nodeType === 3 || a.nodeType === 4 ? a.data : "";
        if (a.nodeType === 1) {
          if (a.textContent !== undefined) return a.textContent = b;
          var c = a.ownerDocument.createTextNode(b);
          return a.appendChild(c), b
        }
        if (a.nodeType === 3) return a.data = b
      },
  ...
  "browser/dom": function(a, b, c) {
    ...
    f.hasAttribute = function(a, b) {
      ...
      var b = [];
      for (var c = 0; c < a.childNodes.length; c++) b.push(e(a.childNodes, c));
      return b
  ...
  "core/scxml/SCXML": function(a, b, c) {
    ...
    function k(a, b) {
      this.model = a, this.opts = b, this.opts.log = this.opts.log || h.platform.log, this.opts.StateIdSet = this.opts.StateIdSet || d, this.opts.EventSet = this.opts.EventSet || d, this.opts.TransitionPairSet = this.opts.TransitionPairSet || d, this.opts.priorityComparisonFn = this.opts.priorityComparisonFn || i(this.opts.model), this.opts.retrace = false, this._sessionid = this.opts.sessionid || "", this._configuration = new this.opts.BasicStateSet, this._historyValue = {}, this._innerEventQueue = [], this._isInFinalState = !1, this._timeoutMap = {}, this._listeners = []
    }

    function l(a, b) {
      b = b || {}, f(b), this._isStepping = !1, this._send = b.send || this._send, this._cancel = b.cancel || this._cancel, k.call(this, a, b)
    }
    "use strict";
    ...
    k.prototype = {
      start: function() {
        j && h.platform.log("performing initial big step"), this._configuration.add(this.model.root.initial);
        ...
        return this._actions = d.actions, this._datamodel = d.datamodel, this._performBigStep(), this.getConfiguration()
      },
      _getOrSetData: function(a, b, c) {
        var d = this._datamodel[b];
        if (!d) throw new Error("Variable " + b + " not declared in datamodel.");
        return d[a](c)
      },
      _getData: function(a) {
      ...
      _performSmallStep: function(a, b) {
        ...
        if (!c.isEmpty()) {
          ...
          var d = new this.opts.TransitionSet(c.iter().filter(function(a) {
            ...
            l = k[0],
            m = k[1];
          j && h.platform.log("basicStatesExited ", g), j && h.platform.log("basicStatesEntered ", l), j && h.platform.log("statesExited ", i), j && h.platform.log("statesEntered ", m);
          var n = new this.opts.EventSet;
          j && h.platform.log("executing state exit actions"), i.forEach(function(c) {
            (j || this.opts.logStatesEnteredAndExited) && h.platform.log("exiting ", c.id), this._listeners.forEach(function(a) {
          ...
          j && h.platform.log("executing transitition actions"), o.forEach(function(c) {
      ...
      _getStatesEntered: function(a) {
        ...
        var c;
        while (c = b.statesToProcess.pop()) this._addStateAndDescendants(c, b);
      ...
      _addStateAndAncestors: function(a, b, c) {
        this._addStateAndDescendants(a, c), this.opts.model.getAncestors(a, b).forEach(function(a) {
      ...
      _addStateAndDescendants: function(a, b) {
        if (b.statesProcessed.contains(a)) return;
        ...
        })) : a.kind === e.COMPOSITE ? b.statesToProcess.push(a.initial) : (a.kind === e.INITIAL || a.kind === e.BASIC || a.kind === e.FINAL) && b.basicStatesToEnter.add(a)), b.statesProcessed.add(a)
      },
      _selectTransitions: function(a, b) {
        if (this.opts.onlySelectFromBasicStates) var c = this._configuration.iter();
        else {
          var d = new this.opts.StateSet;
          this._configuration.iter().forEach(function(a) {
            d.add(a), this.opts.model.getAncestors(a).forEach(function(a) {
              d.add(a)
            })
          }, this), c = d.iter()
        }
        ...
        return j && h.platform.log("priorityEnabledTransitions", n), n
      },
      _selectPriorityEnabledTransitions: function(a) {
        var b = new this.opts.TransitionSet,
          ...
          d = c[0],
          e = c[1];
        b.union(d), j && h.platform.log("enabledTransitions", a), j && h.platform.log("consistentTransitions", d), j && h.platform.log("inconsistentTransitionsPairs", e), j && h.platform.log("priorityEnabledTransitions", b);
        while (!e.isEmpty()) a = new this.opts.TransitionSet(e.iter().map(function(a) {
      ...
      _isArenaOrthogonal: function(a, b) {
        ...
        return j && (h.platform.log("transition scopes", a.scope.id, a.scope.id), h.platform.log("transition scopes are orthogonal?", c)), c
      },
    ...
    }, l.prototype = Object.create(k.prototype), l.prototype.gen = function(a, b) {
      var c;
      switch (typeof a) {
        case "string":
          c = {
          ...
          break;
        case "object":
          if (typeof a.name != "string") throw new Error('Event object must have "name" property of type string.');
          c = a;
          break;
        default:
          throw new Error("First argument to gen must be a string or object.")
      }
      if (this._isStepping) throw new Error("gen called before previous call to gen could complete. If executed in single-threaded environment, this means it was called recursively, which is illegal, as it would break SCION step semantics.");
      return this._isStepping = !0, this._performBigStep(c), this._isStepping = !1, this.getConfiguration()
    }, l.prototype._send = function(a, b) {
      var c, d, e = this;
      if (!h.platform.setTimeout) throw new Error("setTimeout function not set");
      j && h.platform.log("sending event", a.name, "with content", a.data, "after delay", b.delay), c = function() {
        return e.gen(a)
      }, d = h.platform.setTimeout(c, b.delay);
      if (b.sendid) return this._timeoutMap[b.sendid] = d
    }, l.prototype._cancel = function(a) {
      if (!h.platform.clearTimeout) throw new Error("clearTimeout function not set");
      if (a in this._timeoutMap) return j && h.platform.log("cancelling ", a, " with timeout id ", this._timeoutMap[a]), h.platform.clearTimeout(this._timeoutMap[a])
    }, c.exports = {
      SCXMLInterpreter: k,
      SimpleInterpreter: l
    }
  ...
  "core/scxml/json2model": function(a, b, c) {
    function f(a) {
      ...
      a.states.forEach(function(a) {
        ...
        var e = [];
        d.onentry && (d.onentry = b(d.onentry)), d.onexit && (d.onexit = b(d.onexit)), d.transitions.forEach(function(a) {
          a.actions && (a.actions = b(a.actions)), a.lcca && (a.lcca = f[a.lcca]), a.scope = f[a.scope]
        }), d.initial = f[d.initial], d.history = f[d.history], d.children = d.children.map(c), d.parent = f[d.parent], d.ancestors && (d.ancestors = d.ancestors.map(c)), d.descendants && (d.descendants = d.descendants.map(c)), d.transitions.forEach(function(a) {
          a.source = f[a.source], a.targets = a.targets && a.targets.map(c)
        })
      }), a.root = f[a.root];
    ...
    function g(a, b) {
      ...
      try {
        ...
        throw d.platform.log("Failed to evaluate action factory."), d.platform.log("Generated js code to evaluate\n", c), e
      }
    }
    "use strict";
    var d = b("../../platform"),
    ...
    if (b.main === c) {
      ...
      if (h === "-") {
        var j = "";
        process.stdin.resume(), process.stdin.on("data", function(a) {
          j += a
    ...
    var d = b("./state-kinds-enum"),
      e = {
        getAncestors: function(a, b) {
          var c, d, e;
          return d = a.ancestors.indexOf(b), d > -1 ? a.ancestors.slice(0, d) : a.ancestors
        },
  ...
  "core/scxml/set/ArraySet": function(a, b, c) {
    function d(a) {
      a = a || [], this.o = [], a.forEach(function(a) {
        this.add(a)
      }, this)
    }
    "use strict", d.prototype = {
      add: function(a) {
        if (!this.contains(a)) return this.o.push(a)
      },
      remove: function(a) {
      ...
      union: function(a) {
        return a = a.iter ? a.iter() : a, a.forEach(function(a) {
          this.add(a)
        }, this), this
      },
  ...
  "core/scxml/setup-default-opts": function(a, b, c) {
    ...
    c.exports = function(a) {
      return a = a || {}, a.TransitionSet = a.TransitionSet || e, a.StateSet = a.StateSet || e, a.BasicStateSet = a.BasicStateSet || e, a.transitionSelector = a.transitionSelector || d, a.model = a.model || f, a
    }
  },
  ...
  "core/util/annotate-scxml-json": function(a, b, c) {
    ...
    function w(a, b) {
      if (f.platform.dom.hasAttribute(a, "event")) {
        var c, e = f.platform.dom.getAttribute(a, "event");
        e === "*" ? c = [e] : c = e.trim().split(/\s+/).map(function(a) {
          var b = a.match(v);
          if (b) {
            var c = b[1];
            if (!b || !c) throw new Error("Unable to parse event: " + a);
            return c
          }
        }), c.filter(function(a) {
          return a !== "*"
        ...
        if (c.indexOf(undefined) > -1) throw new Error("Error parsing event attribute attributes.event")
      }
      var g = {
      ...
      return f.platform.dom.getElementChildren(a).length && (g.actions = d.gen.parentToFnBody(a)), m.push(g), g
    }
    ...
    function x(a, b) {
      f.platform.dom.getChildren(a).filter(function(a) {
        ...
        if (f.platform.dom.hasAttribute(a, "id")) {
          var b, c = f.platform.dom.getAttribute(a, "id");
          if (f.platform.dom.hasAttribute(a, "expr")) b = {
            ...
            } else g = f.platform.dom.textContent(a), b = g.length ? {
              content: g,
              type: "text"
            } : null
          }
    ...
    function y(a, b) {
      ...
      n[c] = m, b.length && (m.parent = b[b.length - 1]), l === g.HISTORY && (m.isDeep = f.platform.dom.getAttribute(a, "type") === "deep" ? !0 : !1), m.documentOrder = j.length, j.push(m);
      if (l === g.BASIC || l === g.INITIAL || l === g.HISTORY) m.basicDocumentOrder = k.length, k.push(m);
      m.depth = b.length, m.ancestors = b.slice(), b.forEach(function(a) {
      ...
      var o, p, r = [],
        ...
        z = function(a) {
          var b = y(a, t);
          return m.initial = b.id, s.push(b), u = !0
        };
      f.platform.dom.getElementChildren(a).forEach(function(a) {
        switch (f.platform.dom.localName(a)) {
          ...
          case "onexit":
            ...
            break;
          case "initial":
            if (!!u) throw new Error("Encountered duplicate initial states in state " + m.id);
            z(a);
            break;
          case "history":
            var b = y(a, t);
            m.history = b.id, s.push(b);
            break;
          case "datamodel":
            x(a, t);
            break;
      ...
      if (!u && f.platform.dom.localName(a) !== "parallel") {
        var A = f.platform.dom.hasAttribute(a, "initial"),
          C = function(a) {
            ...
            return f.platform.dom.setAttribute(c, "target", a), f.platform.dom.appendChild(b, c), z(b)
          };
        A ? C(f.platform.dom.getAttribute(a, "initial")) : v && C(v.id)
      }
      return m.onexit = o, m.onentry = p, m.transitions = r.map(function(a) {
    ...
    function B(a) {
      return A[a] = A[a] || 0, "" + z + "-" + a + "-" + A[a]++
    }

    function C(a, b) {
      var c, d, e;
      e = [], a.ancestors.forEach(function(a) {
        d = n[a], d.kind === g.COMPOSITE && d.descendants.indexOf(b.id) > -1 && e.push(a)
      });
      if (!e.length) throw new Error("Could not find LCCA for states.");
      return e[0]
    }
    ...
    var d = b("./code-gen"),
      ...
      i = h.concat("scxml"),
      j, k, l, m, n, o, p, q, r = a.transformAndSerialize = r = function(a) {
        ...
        return j.forEach(function(a) {
          ...
          var b = n[a.source],
            c = a.targets.map(function(a) {
              var b = n[a];
              if (!b) throw new Error("Transition targets state id '" + a + "' but state does not exist.");
              return b
            });
          a.lcca = C(b, c[0])
  ...
  "core/util/code-gen": function(a, b, c) {
    ...
    function g(a) {
      ...
      if (!b || !c) throw new Error("Element " + d.platform.dom.namespaceURI(a) + ":" + d.platform.dom.localName(a) + " not yet supported");
      return c(a)
    }
    ...
    function q(a) {
      ...
      if (e.length) return e = e[0], d.platform.dom.getAttribute(e, "type") === "application/json" ? d.platform.dom.textContent(e) : JSON.stringify(d.platform.dom.textContent(e));
      if (d.platform.dom.hasAttribute(a, "contentexpr")) return d.platform.dom.getAttribute(a, "contentexpr");
      var f = "{";
        ...
        "": {
          ...
          if: function(a) {
            ...
            for (var e = 0; e < c.length; e++) {
              var f = c[e];
              if (d.platform.dom.localName(f) === "elseif" || d.platform.dom.localName(f) === "else") break;
              b += g(f) + "\n;;\n"
            }
            for (; e < c.length; e++) {
              f = c[e];
              if (d.platform.dom.localName(f) === "elseif") b += "}else if(" + d.platform.dom.getAttribute(f, "cond") + "){\n";
              else {
          ...
          log: function(a) {
            var b = [];
            return d.platform.dom.hasAttribute(a, "label") && b.push(JSON.stringify(d.platform.dom.getAttribute(a, "label"))), d.platform.dom.hasAttribute(a, "expr") && b.push(d.platform.dom.getAttribute(a, "expr")), "$log(" + b.join(",") + ");"
          },
          raise: function(a) {
  ...
  "core/util/docToModel": function(a, b, c) {
    ...
    function i(a, b) {
      var c;
      if (f.platform.url.resolve) c = f.platform.url.resolve(a, b);
      else {
    ...
    function j(a, b, c, d) {
      var e = [],
        g = [],
        h = 0;
      k(b.documentElement, e), e.length ? e.forEach(function(b, j) {
        var k = f.platform.dom.getAttribute(b, "src");
        a && (k = i(a, k)), f.platform.getResourceFromUrl(k, function(a, c, i) {
          a ? (f.platform.log("Error downloading document " + k + " : " + a.message), g.push({
            url: k,
            err: a
          })) : f.platform.dom.textContent(b, c), ++h, h == e.length && d(g.length ? g : null)
        }, c)
      }) : d()
    }

    function k(a, b) {
      (f.platform.dom.localName(a) === "script" || f.platform.dom.localName(a) === "data") && f.platform.dom.hasAttribute(a, "src") && b.push(a), f.platform.dom.getElementChildren(a).forEach(function(a) {
        k(a, b)
      })
    }
    "use strict";
  ...
  platform: function(a, b, c) {
    ...
    "use strict";
    var g;
    f() ? c.exports = b("./browser/platform") : e() ? c.exports = b("./node/platform") : d() && (c.exports = b("./rhino/platform"))
  },
  scion: function(a, b, c) {
    function g(a, b, c) {
      if (!d.platform.getDocumentFromUrl) throw new Error("Platform does not support getDocumentFromUrl");
    ...
    function h(a, b, c) {
      if (!d.platform.getDocumentFromFilesystem) throw new Error("Platform does not support getDocumentFromFilesystem");
    ...
    function i(a, b, c) {
      if (!d.platform.parseDocumentFromString) throw new Error("Platform does not support parseDocumentFromString");
