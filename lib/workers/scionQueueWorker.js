 ...
 *
 * @private
 * @type {[engineId: number]: JobQueue}
 *
 * where JobQueue is an Array with additional property JobQueue.readyForJob {Boolean}
 */
var queues = {};
 ...
 * @param {String} evt.data.command the message type / name (e.g. <tt>finishedJob</tt>)<br>
 * @param {Number | String} evt.data.engineId Number the engine's ID for which this job should be queued
 * @param {Object} [evt.data.job] the job object (e.g. in case of command <tt>queueJob</tt> the job that will be queued)
 * @param {Object} [evt.data.job.engineId] Number | String the engine's ID for which this job should be queued
 * @param {Object} [evt.data.job.event] String OPTIONAL the event name (must be present for command "queueJob")
 * @param {Object} [evt.data.job.eventData] any OPTIONAL event data
 *
 * @public
 */
 ...
 * @return {JobQuee} the queue for the engineId (if isCreate is FALSE, and no queue exits for the engineId, it will return UNDEFINED)
 */
 ...
 * @function
 * @private
 * @param {JobQueue} queue the job queue
 */
