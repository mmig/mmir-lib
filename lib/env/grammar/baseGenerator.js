define(['mmirf/codeGenUtils', 'mmirf/util/extend'], function(codeGenUtils, extend){
/**
 ...
 * @hideconstructor
 *
 * @param {mmir.tools.Logger} logger the logger instance of the specific engine/generator
...
return function BaseGenerator(logger, engineId) {
	var gen = {
		...
		doParseUtterance: function(utterance_name, utterance_def){
			...
			var vars = {};
			var semantic  = self.doCreateSemanticInterpretationForUtterance(utterance_name, utterance_def, vars);
			...
			for(var index=0,size=phrases.length; index < size; ++index){
				...
				var semantic_interpretation = self.doCreateSemanticInterpretationForPhrase(
						utterance_name.toLowerCase(), phrase, semantic, vars
				);
		 ...
		 * @param {String} utterance_name the utterance name
		 * @param {UtteranceJson} utterance_def the JSON definition for the utterance
		 * @param {Object} var_map INOUT variable that will contain the variable names that were referenced/used in the semantic definition:
		 *                         if variable index has an index-reference, it will be included as string, otherwise the entry is set to <code>true</code>, i.e.
		 *                         <pre>{[varName: string]: true | string}</pre>
		 *
		 * @returns {String} the code for the utterance's semantic interpretation processing
		 */
		doCreateSemanticInterpretationForUtterance: function(utterance_name, utterance_def, var_map){
	...
	//							);
					...
					var_map[variable_name.toLowerCase()] = variable_index? variable_index[1] : true;
