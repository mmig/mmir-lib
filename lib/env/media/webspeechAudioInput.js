...
return {
	...
	initialize: function(callBack, __mediaManager, ctxId, moduleConfig){
		...
		if(!SpeechRecognitionImpl){
			...
			callBack(
				/** @class mmir.env.media.WebspeechAudioInput.DisabledWebspeechAudioInput */
				{
				/**
				 * create & show error dialog
				 * @private
				 * @memberOf mmir.env.media.WebspeechAudioInput.DisabledWebspeechAudioInput.prototype
				 */
				__triggerError: function(name, options, successCallback, failureCallback){
					...
					if(failureCallback)
						failureCallback('Failed to execute ' +name+ '().\n'+ msg);
					else
						logger.error('Failed to execute ' +name+ '().\n'+ msg);
				}
			/**
			 * will trigger error callback (or log error)
			 * @public
			 * @memberOf mmir.env.media.WebspeechAudioInput.DisabledWebspeechAudioInput.prototype
			 * @see mmir.MediaManager#startRecord
			 */
			, startRecord: function(options, successCallback, failureCallback){
				this.__triggerError('startRecord', options, successCallback, failureCallback);
			}
			/**
			 * will trigger error callback (or log error)
			 * @public
			...
			, stopRecord: function(options, successCallback,failureCallback){
				this.__triggerError('stopRecord', options, successCallback, failureCallback);
			}
			/**
			 * will trigger error callback (or log error)
			 * @public
			...
			, recognize: function(options, successCallback,failureCallback){
				this.__triggerError('recognize', options, successCallback, failureCallback);
			}
			/**
			 * will trigger error callback (or log error)
			 * @public
			...
			, cancelRecognition: function(successCallback,failureCallback){
				this.__triggerError('cancelRecognition', successCallback, failureCallback);
			}
			/**
			 * will trigger error callback (or log error)
			 * @public
			...
			, getRecognitionLanguages: function(successCallback,failureCallback){
				this.__triggerError('getRecognitionLanguages', successCallback, failureCallback);
			}
			});
			return;////////////////////// EARLY EXIT ///////////////////////////
		}
