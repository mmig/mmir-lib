...
define(['mmirf/languageManager', 'mmirf/util/toArray'], function(lang, toArray){
	 ...
	 * @private
	 */
	var _getFixedVoice = function(options){

		var voiceParam = _getVoiceParam(options);

		var voice;
		if(typeof voiceParam === 'string'){
			var filterOpt = /^male|female$/i.test(voiceParam)? {gender: voiceParam} : {name: voiceParam};
			var voices = _getVoiceList().filter(_createVoiceFilter({details: filterOpt}));
			if(voices.length > 0 && speechSynthesis){
				var id = voices[0].name;
				voice = speechSynthesis.getVoices().find(function(v){ return v.voiceURI === id});
			}
		} else {
			voice = voiceParam
		}
	...
	var _createUtteranceAudio = function(text, options, onEnd, failureCallback, onCanPlay){
		...
		try {
			...
			var voice = _getFixedVoice(options);
