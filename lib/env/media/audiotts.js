...
//								isReady = true;//DISABLED -> EXPERIMENTAL: command-queue feature.
		...
		var initImpl = function(impl){
			...
			initSynthesizer = impl.getInitFunc? impl.getInitFunc() : _logger.info('no getInitFunc() function: will not be able to re-initialize '+pluginName) && void(0);
			destroySynthesizer = impl.getDestroyFunc? impl.getDestroyFunc() : _logger.info('no getDestroyFunc() function: will not be able to destroy '+pluginName) && void(0);
...
//						return;
			...
			/**
			 * destroy speech synthesizer instance and freeing up system resources.
			 *
					...
					destroySynthesizer(function(err){
						if(err){
							if(failureCallback){
								...
								(_logger? _logger : console).error('failed to destroy synthesizer instance: '+err, err);
							}
		...
		var handleError = function error(err){
			...
			(_logger? _logger : console).error('failed to load media plugin (file) '+implPath+': '+err, err);

			//invoke the passed-in initializer-callback and supply list of non-functional functions:
			callBack({}, {
				mod: implPath,
				disabled: ['tts', 'cancelSpeech', 'setTextToSpeechVolume', 'getSpeechLanguages', 'getVoices', 'destroySpeech'],
				message: err
			});
		};
		...
		if(typeof WEBPACK_BUILD !== 'undefined' && WEBPACK_BUILD){
			...
			require([implPath], processLoaded, function(_err){
