...
function initMmir(globalCtx) {
	...
	if(globalCtx[CORE_NAME]){

		//if globalCtx[CORE_NAME] is the core-module: register it and return
		//(note: if it is not the core-module, its properties will be merged/copied to the core-module -> see below)
		if(typeof globalCtx[CORE_NAME].startModule === 'string' && typeof define === 'function'){
			define(function(){ return globalCtx[CORE_NAME]; });
			return globalCtx[CORE_NAME];
		}
	}
...
//	})();
	...
	//if globalCtx[CORE_NAME] already exists:
	//  copy all its properties to the new core-mmir object
	if(globalCtx[CORE_NAME]){
		for(var p in globalCtx[CORE_NAME]){
			if(globalCtx[CORE_NAME].hasOwnProperty(p) && typeof mmir[p] === 'undefined'){
				mmir[p] = globalCtx[CORE_NAME][p];
			}
		}
	}

	//export core-module into global namespace:
	globalCtx[CORE_NAME] = mmir;
