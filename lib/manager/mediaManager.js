	...
	var loadPlugin = function loadPlugin(filePath, successCallback, failureCallback, execId, config, pluginIndex){
		try {
			...
			var processLoaded = function(newMediaPlugin){
				 ...
				 * @param  {promise | {disabled: boolean | string[] | {[func: string]: Function}, mod?: string, message?: string, errorCallbackNme?: string}} [nonFunctional] OPTIONAL
				 ...
				 */
				var onInitialized = function(exportedFunctions, /* {disabled: boolean | string[] | {[func: string]: Function}, mod?: string, message?: string, errorCallbackName?: string} | promise */nonFunctional){
...
//					    								logger.log('executing '+context+'.'+name+', in context '+mediaManagerInstance,mediaManagerInstance);//DEBUG
				...
				newMediaPlugin.initialize(onInitialized, instance, execId, config);
...
//    	create : function(){ return this.init.apply(this, arguments); },
		 ...
		 *
		 */
		loadFile: function(filePath, successCallback, failureCallback, execId, config){
			if (instance=== null) {
				this.init();
			}
