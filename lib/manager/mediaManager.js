...
define(['mmirf/util/deferred', 'mmirf/util/extend', 'mmirf/util/isArray', 'mmirf/resources', 'mmirf/configurationManager', 'mmirf/logger', 'mmirf/events', 'module'],
	 ...
	 *
	 * supported environments: <default> ('browser') | 'cordova' | 'capacitor'
	 *
	 * TODO do we need to differentiate between more environments?
	 *
	 * @param  {String} [platform] one of 'browser' | 'cordova' | 'capacitor' (DEFAULT: 'browser')
	 * @return {Array<PluginEntry>} the list of required PluginEntry objects for the env
	 *
	 * @private
	 * @memberOf mmir.MediaManager#
	 */
	function getRequiredPlugins(platform){
		switch(platform){
			case 'cordova':
				return [{mod: 'cordovaAudio', type: 'audio'}];
			case 'capacitor':
			case 'browser':
			default:
				return [{mod: 'webAudio', type: 'audio'}];
		}
	}
	 ...
	 */
	var _defaultPlugins = {
		'browser': getRequiredPlugins().concat([
					{mod: 'webspeechAudioInput', type: 'asr'},
					{mod: 'audiotts', config: 'ttsMary', type: 'tts'}
		]),
		'cordova': getRequiredPlugins('cordova').concat([
					{mod: 'asrAndroid', type: 'asr'},
					{mod: 'audiotts', config: 'ttsMary', type: 'tts'}
		]),
		'capacitor': getRequiredPlugins('capacitor').concat([
					{mod: 'asrAndroid', type: 'asr'},
					{mod: 'audiotts', config: 'ttsMary', type: 'tts'}
		])
	};
...
///////////////////////////// MediaManager "managing" functions: /////////////////////////////
	...
	function getPluginsToLoad(configurationName){//if configurationName is omitted, then it is automatically detected
		...
		if(!env){
			...
			if(envSetting === 'cordova'){
				...
				//  if there is a default config, then the env will also be a default one
			...
			if(!env){
				if (res.isCapacitorEnv()) {
					env = 'capacitor';
				} else if (res.isCordovaEnv()) {
					env = 'cordova';
				} else {
					env = 'browser';
				}
	...
	function verifyRequiredPlugins(plugins, requiredPlugins){
		var isVerified = true;
		var entry, mod, rq, ctx;
		for(var i=plugins.length-1; i >= 0; --i){
			entry = plugins[i];
			mod = typeof entry === 'string'? entry : entry.mod;
			...
			for(var j=requiredPlugins.length-1; j >= 0; --j){
				...
				if((mod === rq.mod || rq.type === entry.type) && ((!ctx && !rq.ctx) || (ctx === rq.ctx))){
					logger.info('replacing default plugin '+rq.mod+' (type: '+rq.type+') with custom configuration ' + mod);
					requiredPlugins.splice(j, i);
				}
			}
	...
	function loadAllPlugins(pluginArray, successCallback, failureCallback){
		...
		var nativePlatform = res.isCapacitorEnv()? 'capacitor' : res.isCordovaEnv()? 'cordova' : void(0);
		var requiredPlugins = getRequiredPlugins(nativePlatform);
		...
		for(var i = 0, size = count; i < size; ++i){
			...
			if(i === size - 1 && requiredPlugins && requiredPlugins.length > 0){
				...
				if(logger.isi()) logger.info('required plugins for '+(nativePlatform? nativePlatform : 'default')+' environment were not explicitly specified, now loading required plugins: '+JSON.stringify(requiredPlugins));
