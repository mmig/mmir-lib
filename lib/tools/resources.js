...
function(
	 ...
	 * @default ""
	 */
	var basePath;//default value is set via resetBasePath()

	/**
	 * the base path of the (i.e. this) library
	 * @private
	 * @memberOf Resources#
	 * @default module.config().mmirBasePath or "mmirf/"
	 */
	var frameworkBasePath;//default value is set via resetBasePath()

	// do initialize basePath and frameworkBasePath with default values
	resetBasePath();
	...
	function resetBasePath(){
		basePath = "";
		frameworkBasePath = typeof _modConf.mmirBasePath === 'string'? _modConf.mmirBasePath : "mmirf/";
	}
	...
	function setBasePath(isBrowserEnvParam){
		...
		if(typeof isBrowserEnvParam === 'string'){
			...
			if(env === 'cordova'){
				...
				if(env === 'default'){
					console.warn('Unknown cordova platform "'+env+'", using default base path ./ (empty)');
				}
			}

			switch(env){
				case 'android':
					basePath = !isBrowserEnvParam.isCapacitorEnv? "file:///android_asset/www/" : "";
					break;
				case 'electron':
				case 'cordova-electron':
				case 'ios':
				case 'default':
				default:
					basePath = "";
		...
		else if (isBrowserEnvParam && isBrowserEnvParam.isCapacitorEnv){
		...
		if(isFinite(frameworkBasePathIndex) && (frameworkBasePathIndex > 0 || basePath.length > 0)){
			...
			if((isBrowserEnvParam && (!isBrowserEnvParam.isAbsolutePath || (typeof isBrowserEnvParam.isAbsolutePath === 'function' && !isBrowserEnvParam.isAbsolutePath(frameworkBasePath))))){
	...
	function constructor(env){
		...
		return {
			 ...
			 * @param {Boolean} [isReset] OPTIONAL if truthy, resource paths will be reset to their default values, before applying the <code>forBrowserParameter</code> parameter<br>
			 * 														NOTE the argument is positional, i.e. <code>forBrowserParameter</code> must also be specified.
			 * @returns {mmir.Resources} the instance of {@link mmir.Resources}
			 * @public
			 *
			 * @memberOf mmir.Resources.prototype
			 */
			init: function(theForBrowserParameter, isReset){
			 ...
			 * @memberOf mmir.Resources.prototype
			 */
			isCapacitorEnv: function(){
				return env.isCapacitorEnv;
			},

			/**
			 * @function
			 * @returns {String}
			 * @values  "cordova" | "browser" | "android" | "ios" | "node" | "electron" | "default" | (or: VALUE set in document's query-parameter "?env=VALUE"
			 * @public
			 *
			 * @memberOf mmir.Resources.prototype
			 */
			getEnv: function(){
				return envInfo && envInfo.envSetting? envInfo.envSetting : 'browser';
			},

			/**
			 * @function
			 * @returns {String}
			 * @values "android" | "ios" | "browser" | "node" | "electron" | "default"
			 * @public
			 *
			 * @memberOf mmir.Resources.prototype
			 */
			getEnvPlatform: function(){
				return envInfo && envInfo.platform;
			}
		};//END: return{}
